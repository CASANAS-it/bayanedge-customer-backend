...
const customControllers = {
    init: router => {
        const baseUrl = `${Properties.api}/revenue`
    ...
    get: async (req, res) => {
        try {
            ...
            revenueService.getAll(limit, offset,client_id).then(data => {
    ...
    getById: async (req, res) => {
        try {
            ...
            res.send(
                new CommonMessage({
                    data: await revenueService.getById(id)
                })
            )
        } catch (err) {
            const safeErr = ErrorManager.getSafeError(err)
    ...
    save: async (req, res) => {
        try {
            var data;
            if (!req.body.id) {
                data = await revenueService.create(req.body)
            } else {
                data = await revenueService.update(req.body)
            }
            res.send(
                new CommonMessage({
                    data: data
    ...
    delete: async (req, res) => {
        try {
            var data = await revenueService.delete(req.body)
