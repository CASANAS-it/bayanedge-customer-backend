...
const customControllers = {
    init: router => {
        const baseUrl = `${Properties.api}/equity`
    ...
    get: async (req, res) => {
        try {
            ...
            equityService.getAll(limit, offset,client_id).then(data => {
    ...
    getById: async (req, res) => {
        try {
            ...
            res.send(
                new CommonMessage({
                    data: await equityService.getById(id)
                })
            )
        } catch (err) {
            const safeErr = ErrorManager.getSafeError(err)
    ...
    save: async (req, res) => {
        try {
            var data;
            if (!req.body.id) {
                data = await equityService.create(req.body)
            } else {
                data = await equityService.update(req.body)
            }
            res.send(
                new CommonMessage({
                    data: data
    ...
    delete: async (req, res) => {
        try {
            var data = await equityService.delete(req.body)
