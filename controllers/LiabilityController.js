...
const customControllers = {
    init: router => {
        const baseUrl = `${Properties.api}/liability`
    ...
    get: async (req, res) => {
        try {
            ...
            liabilityService.getAll(limit, offset,client_id).then(data => {
    ...
    getById: async (req, res) => {
        try {
            ...
            res.send(
                new CommonMessage({
                    data: await liabilityService.getById(id)
                })
            )
        } catch (err) {
            const safeErr = ErrorManager.getSafeError(err)
    ...
    save: async (req, res) => {
        try {
            var data;
            if (!req.body.id) {
                data = await liabilityService.create(req.body)
            } else {
                data = await liabilityService.update(req.body)
            }
            res.send(
                new CommonMessage({
                    data: data
    ...
    delete: async (req, res) => {
        try {
            var data = await liabilityService.delete(req.body)
